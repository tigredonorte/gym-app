...
export class AuthService {
  ...
  async logout({ sessionId, email }: LogoutDto, userData: IRequestInfo['userData']) {
    await this.sessionService.removeSession(sessionId);
    const user = await this.userService.findByEmail(email);
    if (!user || !user.id) {
      throw new Error('User not found');
    }
    await this.authEventsService.emitLogout({ sessionId, user: { email, name: user.name, id: `${user.id}` }, userData });
    return {};
  }
  ...
  async login({ email, password }: LoginDto, userData: IRequestInfo['userData']) {
    ...
    await this.authEventsService.emitLogin({ user: { email, name, id: `${id}` }, userData, sessionId });
    return {
      ...result,
      sessionId,
    };
